export function countInversions(
  arr: Array<number>
): { arr: Array<number>; inversions: number } {
  console.log("subproblem", arr);
  if (arr.length === 1) {
    return { arr: arr, inversions: 0 };
  }
  const resultArr = Array.of(6);
  const { arr: leftPartArr, inversions: leftSideInversions } = countInversions(
    arr.slice(0, arr.length / 2)
  );
  const {
    arr: rightPartArr,
    inversions: rightSideInversions,
  } = countInversions(arr.slice(arr.length / 2, arr.length));
  let l_Indx = 0,
    r_Indx = 0;
  ...
  for (let index = 0; index < arr.length; index++) {
    ...
    if (
      (leftPartArr[l_Indx] < rightPartArr[r_Indx] && !reachedEndOfLeft) ||
      reachedEndOfRight
    ) {
    ...
    if (
      (leftPartArr[l_Indx] >= rightPartArr[r_Indx] && !reachedEndOfRight) ||
      reachedEndOfLeft
    ) {
