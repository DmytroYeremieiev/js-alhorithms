...
describe('diffArray', () => {
  ...
  it('should return ["diorite", "pink wool"]', () => {
    ...
    expect(result)
      .to.have.lengthOf(2)
      .and.to.include.members(['diorite', 'pink wool']);
  });
  it('should return ["piglet", 4]', () => {
    const result = diffArray([1, 'calf', 3, 'piglet'], [1, 'calf', 3, 4]);
    expect(result)
      .to.have.lengthOf(2)
      .and.to.include.members(['piglet', 4]);
  });
  it('should return an empty array', () => {
    const result = diffArray(['andesite', 'grass', 'dirt', 'dead shrub'], ['andesite', 'grass', 'dirt', 'dead shrub']);
  ...
  it('should return ["snuffleupagus", "cookie monster", "elmo"]', () => {
    ...
    expect(result)
      .to.have.lengthOf(3)
      .and.to.include.members(['snuffleupagus', 'cookie monster', 'elmo']);
  });
  it('should return [1, "calf", 3, "piglet", 7, "filly"]', () => {
    const result = diffArray([1, 'calf', 3, 'piglet'], [7, 'filly']);
    expect(result)
      .to.have.lengthOf(6)
      .and.to.include.members([1, 'calf', 3, 'piglet', 7, 'filly']);
  });
});
